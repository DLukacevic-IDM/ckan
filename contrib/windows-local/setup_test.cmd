...
ECHO run the command below in the virtualenv that you created:
ECHO nosetests -v --ckan --ckan-migration --reset-db -d ckan ckanext
ECHO if you want to run individual tests add --tests=TestAppDispatcher.test_ask_around_flask_core_and_pylons_extension_route
